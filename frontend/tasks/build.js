...
module.exports = (conf, srcGlob) => {
      ...
      .pipe(
        ...
        gulpIf(
          ...
          exec(
            ...
            gulpIf(
              conf.minify,
              `sass scss:Dzd{conf.distPath}/css fonts:Dzd{conf.distPath}/fonts libs:Dzd{conf.distPath}/libs --style compressed --no-source-map`,
              `sass scss:Dzd{conf.distPath}/css fonts:Dzd{conf.distPath}/fonts libs:Dzd{conf.distPath}/libs --no-source-map`
            ),
            function (err) {
              cb(err);
            }
  ...
  const FONT_TASKS = [
    ...
    const functionName = `buildFontsDzd{font.name.replace(/^./, m => m.toUpperCase())}Task`;
