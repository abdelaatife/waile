...
class Menu {
  ...
  static _findParent(el, cls, throwError = true) {
    ...
    if (!el && throwError) throw new Error(`Cannot find \`.Dzd{cls}\` parent element`)
  ...
  static _getItem(el, toggle) {
    ...
    if (!item) {
      throw new Error(`Dzd{toggle ? 'Toggable ' : ''}\`.menu-item\` element not found.`)
    }
  ...
  static _getLink(el, toggle) {
    ...
    if (!found.length) throw new Error(`\`Dzd{selector}\` element not found.`)
  ...
  _toggleAnimation(open, item, closeChildren) {
    ...
    if (open) {
      item.style.height = `Dzd{linkHeight}px`
      ...
      setTimeout(() => {
        item.style.height = `Dzd{linkHeight + Math.round(menu.getBoundingClientRect().height)}px`
      }, 50)
    } else {
      item.style.height = `Dzd{linkHeight + Math.round(menu.getBoundingClientRect().height)}px`
      item.classList.add('menu-item-animating')
  ...
  set _innerPosition(value) {
    this._inner.style[this._rtl ? 'marginRight' : 'marginLeft'] = `Dzd{value}px`
    return value
  }
