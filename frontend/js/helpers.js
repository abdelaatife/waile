...
function requiredParam(name) {
  throw new Error(`Parameter requiredDzd{name ? `: \`Dzd{name}\`` : ''}`)
}
...
const Helpers = {
  ...
  _triggerWindowEvent(name) {
    ...
    if (document.createEvent) {
      ...
      window.fireEvent(`onDzd{name}`, document.createEventObject())
    }
  },
  ...
  // Trigger event
  _triggerEvent(name) {
    this._triggerWindowEvent(`layoutDzd{name}`)
  ...
  ajaxCall(url) {
    return new Promise((resolve, reject) => {
      ...
      req.onerror = e => reject(Error(`Network Error: Dzd{e}`))
      req.send()
    })
  },
